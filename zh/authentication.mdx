...
| `group_id` | 该 key 可触达的 channel 路由分组。选 `claude-default` 意味着 key 只能调 Claude 池里服务的模型；`volcengine-default` 只能路由到 DeepSeek + Doubao。详见 [Channels](/zh/concepts/channels)。 | 必填（每 key 单值） |
